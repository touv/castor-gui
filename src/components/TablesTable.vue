...
export default {
			...
			let url2 = self.store.serverHost + '/' + self.store.currentTable._id + '/$count';
				self.$http.get(url1).then(function (response) {
					if (Array.isArray(response.data)) {
						...
						self.maxpage = Math.round(response.data[0].value / self.pagesize)
					}
				}, console.error)
  ...
  methods: {
    load(done) {
      ...
      let url2 = self.store.serverHost + '/' + self.store.currentTable.name + '/*?' + MQS.stringify(query);
    ...
    loadMore () {
      ...
      if (self.page < self.maxpage) {
        self.busy = true;
        console.log('Load more page #', self.page,'/', self.maxpage);
        self.load(() => {
          self.busy = false;
        })
      }
